...
"""MSA Shannon entropy calculation application."""
...
__all__ = ['evol_entropy']

APP = DevelApp('entropy', 'calculate Shannon entropy')

APP.setExample(
"""This application calculates Shannon entropy for a refined multiple \
sequence alignment.  Following example will save entropy data and plot \
using default options:

    $ evol entropy piwi_refined.slx -E""", [])
...
APP.addArgument('-p', '--prefix',
    ...
    help='output filename prefix, default is '
         'msa filename with _entropy suffix',
    type=str,
    metavar='STR',
    group='output')
...
APP.addFigure('-E', '--entropy-plot', 
    dest='figent', 
    action='store_true', 
    help='save Shannon entropy plot')
