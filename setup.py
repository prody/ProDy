...
hpb_cpp = join('prody', 'proteins', 'hpbmodule', 'hpbmodule.cpp')
hpb_f   = join('prody', 'proteins', 'hpbmodule', 'reg_tet.f')

if isfile(hpb_cpp) and isfile(hpb_f):
    EXTENSIONS.append(
        Extension('prody.proteins.hpb',
                  sources=[hpb_cpp, hpb_f],
                  include_dirs=[numpy.get_include()],
                  language='c++',
