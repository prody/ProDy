...
In the interactive usage examples throughout the documentation, the input to 
ProDy functions and classes is the set of atomic coordinates of the query 
protein in PDB file format, or simply the PDB id or sequence of the protein.  
Fast and flexible ProDy parsers are used to retrieve the corresponding dataset 
of PDB files from the PDB FTP server and extract their coordinate data and 
other relevant information. After PCA and NMA calculations, results  
can be plotted and saved as high-quality image files or visualized in VMD
using NMWiz plugin.  
...
We select backbone atoms by passing atom names with :term:`name` keyword:
...
customized force constant functions or a pre-calculated matrix 
...
ANM server (|anm|) as the external software. 
We will use :download:`oanm.eigvals <doctest/oanm.eigvals>` 
