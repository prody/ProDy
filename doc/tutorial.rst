...
variability are extracted using principal component analysis (PCA). 
...
In interactive usage examples throughout the documentation, the input to ProDy 
functions and classes is protein structural data in PDB file format or 
simply a PDB identifier. Fast and flexible ProDy parsers are used to
retrieve PDB files from Protein Data Bank FTP server and extract coordinate 
data and other relevant information. The results then generated by ProDy 
are plotted and saved as high-quality image files, saved in comma-separated 
value formats, or saved in :term:`NMD` format for visualization in VMD.  
