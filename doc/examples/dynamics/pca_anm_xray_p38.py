#!/usr/bin/env python
"""Essential Dynamics Analysis of X-ray structural ensembles

p38 structures are analyzed and essential modes are compared with ANM modes.
...
# The list is update to include corresponding PDB identifiers.
...
# Perform EDA
eda = PCA('p38 essential dynamics')
eda.buildCovariance(ensemble)
eda.calcModes()

# Write essential modes into an NMD file for NMWiz
writeNMD('p38_essential_modes.nmd', eda[:3], ref_structure)
...
# Calculate overlaps between ANM and EDA modes
printOverlapTable(eda[:3], anm[:3])
