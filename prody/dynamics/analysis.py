...
def calcEntropyTransfer(model, ind1, ind2, tau):
    ...
    for k in range(n_modes):
        dummy1 += eigvals[k] * eigvecs[k,ind2] * eigvecs[k,ind2]
        dummy2 += eigvals[k] * eigvecs[k,ind2] * eigvecs[k,ind2] * exp(-eigvals[k]*tau/tau_0)
    T += 0.5 * log(dummy1**2 + dummy2**2)
    ...
    dummy10 = 0 
    for k in range(n_modes):
        dummy1 += eigvals[k] * eigvecs[k,ind1] * eigvecs[k,ind1]
        dummy2 += eigvals[k] * eigvecs[k,ind2] * eigvecs[k,ind2]
        dummy3 += eigvals[k] * eigvecs[k,ind1] * eigvecs[k,ind2]
        dummy4 += eigvals[k] * eigvecs[k,ind2] * eigvecs[k,ind2] * exp(-eigvals[k]*tau/tau_0)
        dummy5 += eigvals[k] * eigvecs[k,ind1] * eigvecs[k,ind2] * exp(-eigvals[k]*tau/tau_0)  
        dummy9 += eigvals[k] * eigvecs[k,ind2] * eigvecs[k,ind2] * exp(-eigvals[k]*tau/tau_0)       
    ...
    T -= 0.5 * log(dummy1*dummy2**2+2*dummy3*dummy4*dummy5-(dummy6**2+dummy7**2)*dummy8-dummy9**2*dummy10)
    T -= 0.5 * log(dummy2)
    T += 0.5 * log(dummy1*dummy2-dummy3**2)
    return T

def calculateAllEntropyTransfer(model, tau):
...
def overallNetEntropyTransfer(model):
    """This function calculates the net entropy transfer for a whole structure 
    with a given time constant tau based on GNM.  
    """
    if not isinstance(model, NMA):
    ...
    tau_max = 20 
    tau_step = 0.5
    taus = np.arange(start=tau_step,stop=tau_max,step=tau_step)
    numTaus = (tau_max / tau_step).astype(int)
    netEntropyTransfer = np.zeros((numTaus,n_atoms,n_atoms))
    LOGGER.timeit('_ent_trans')
    for i in range(numTaus):
        netEntropyTransfer[i,:,:]=calcNetEntropyTransfer(model,taus[i])
