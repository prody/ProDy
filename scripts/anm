...
# TO DO LIST
# enable/disable silent (verbose) mode
# write NMD file
...
def main():
    ...
    parser.add_option('-c','--cutoff',dest="cutoff",type="float",default=15.0,metavar="FLOAT",help="cutoff distance (%default)")
    parser.add_option('-g','--gamma',dest="gamma",type="float",default=1.0,metavar="FLOAT",help="spring constant (%default)")
    parser.add_option('-p','--prefix',dest="prefix",type="string",default="anm_",metavar="STRING",help="prefix for output files (%default)")
...
"""
    ...
    # parameters
    pdbfn = args[0]
    nmodes, select, cutoff, gamma, prefix = opt.nmodes, opt.select, opt.cutoff, opt.gamma, opt.prefix
    ...
    # gnm calculation
    anm = ANM('ANM pdb:%s select:%s cutoff:%0.2f gamma:%0.2f'%(pdb,select,cutoff,gamma))
    anm.buildHessian(pdbselect,cutoff,gamma)
    anm.calcModes(nmodes)
    
    # output
    eigenvaluesfn = "%sevalues.csv"%(prefix)
    eigenvectorsfn = "%sevectors.csv"%(prefix)
    nmdfile = "%snmd.nmd"%(prefix)
